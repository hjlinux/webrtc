...
int getIdLen(const char *data)
{
    int k = 0;
    while((data[k] >= '0' && data[k] <= '9') || (data[k] == '.'))
        k++;
    return k;
}
...
ChannelMember::ChannelMember(DataSocket* socket)
  ...
  char ip_str[20];
  unsigned long addr = socket->get_addr();
  char *p = (char *)&addr;
...
std::string ChannelMember::GetPeerIdHeader() const {
  std::string ret(kPeerIdHeader + /*int2str(id_)*/id_ + "\r\n");
  return ret;
}
...
std::string ChannelMember::GetEntry() const {
  ...
  sprintfn(entry, sizeof(entry), "%s,%s,%d\n",
           name_.substr(0, kMaxNameLength).c_str(), id_.c_str(), connected_);
  return entry;
}
...
ChannelMember* PeerChannel::Lookup(DataSocket* ds) const {
  ...
  std::string id;
  int len = getIdLen(&args[found + ARRAYSIZE(kPeerId) - 1]);
  id.assign(&args[found + ARRAYSIZE(kPeerId) - 1],len);//atoi(&args[found + ARRAYSIZE(kPeerId) - 1]);
...
ChannelMember* PeerChannel::IsTargetedRequest(const DataSocket* ds) const {
  ...
  int len = getIdLen(&path[found]);
  std::string id;
  id.assign(&path[found],len);
...
bool PeerChannel::AddMember(DataSocket* ds) {
...
#if 1
  unsigned long addr = ds->get_addr();
  new_guy->set_addr(addr);
  for (Members::iterator i = members_.begin(); i != members_.end(); ++i) {
    ChannelMember* m = (*i);
    if (m->get_addr() == new_guy->get_addr()) {
      printf("reset: %s\n", m->name().c_str());
      m->set_disconnected();
      i = members_.erase(i);
      Members failures;
  ...
  printf("New member added %s (total=%s): %s\n",ip_str,
...
void PeerChannel::OnClosing(DataSocket* ds) {
  for (Members::iterator i = members_.begin(); i != members_.end(); ++i) {
    ...
    unsigned long addr = ds->get_addr();
    //char *p = (char *)&addr;
    //fprintf(stdout,"oncloseing:%u.%u.%u.%u\n",p[0],p[1],p[2],p[3]);
    if (!m->connected()) {
        printf("onclosed\n");
      i = members_.erase(i);
      Members failures;
