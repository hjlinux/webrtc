...
#if 1
int MainProc::DbDelGroup(void)
{
	char sql1[] = "delete from group1";
	char sql2[] = "delete from group2";
	char sql3[] = "delete from group3";
	char begin[] = "begin transaction";
	char commit[] = "commit transaction";
	int ret;
	zk_sqlite3_exec(begin, NULL, NULL);
	ret = zk_sqlite3_exec(sql1, NULL, NULL);
	ret = zk_sqlite3_exec(sql2, NULL, NULL);
	ret = zk_sqlite3_exec(sql3, NULL, NULL);
	zk_sqlite3_exec(commit, NULL, NULL);
	return ret;
}

//"group1":[[1,"wwww"],[2,"wwqqqq"]]
int MainProc::DbInsertGroup(Json::Value& group, std::string& group_numb)
{
	char buffer[2048];
	Json::Value value;
	int ret, i, id;
	...
	{
		rtc::GetValueFromJsonArray(group, i, &value);
		rtc::GetIntFromJsonArray(value, 0, &id);
		rtc::GetStringFromJsonArray(value, 1, &name);
		memset(buffer, 0, 2048);
		sprintfn(buffer, 2047, "insert into %s values (%d, \"%s\")", 
					group_numb.c_str(), id, name.c_str());
		ret = zk_sqlite3_exec(buffer, NULL, NULL);
	}
	zk_sqlite3_exec(commit, NULL, NULL);
	return 0;
}

int MainProc::DbDelUserList(void)
{
	char sql[] = "delete from user_info";
	int ret;
	ret = zk_sqlite3_exec(sql, NULL, NULL);
	return ret;
}
//["\u6559\u5ba4173","19216813173","online","0","0","0"]
int MainProc::DbInsUserList(Json::Value& user)
{
	char buffer[2048];
	Json::Value value;
	int ret, i, group1, group2, group3;
	...
	{
		rtc::GetValueFromJsonArray(user, i, &value);
		rtc::GetStringFromJsonArray(value, 0, &name);
		rtc::GetStringFromJsonArray(value, 1, &id);
		rtc::GetStringFromJsonArray(value, 2, &status);
		rtc::GetIntFromJsonArray(value, 3, &group1);
		rtc::GetIntFromJsonArray(value, 4, &group2);
		rtc::GetIntFromJsonArray(value, 5, &group3);
		memset(buffer, 0, 2048);
		sprintfn(buffer, 2047, "insert into user_info (name,user_id,state,group1,group2,group3) values (\"%s\", \"%s\", \"%s\", %d, %d, %d)", 
				 name.c_str(), id.c_str(), status.c_str(), group1, group2, group3);
		ret = zk_sqlite3_exec(buffer, NULL, NULL);
	}
	zk_sqlite3_exec(commit, NULL, NULL);
	return 0;
