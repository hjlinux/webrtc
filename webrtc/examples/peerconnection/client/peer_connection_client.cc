...
void PeerConnectionClient::SendConnGetGroup(void)
{
	Json::Value jmessage;
	jmessage["pkgtype"] = "connect";
	jmessage["msgtype"] = "getgroup";
	jmessage["sequence"] = Sequence();
	SendMessage(jmessage);
}

void PeerConnectionClient::SendConnGetList(void)
{
	Json::Value jmessage;
	jmessage["pkgtype"] = "connect";
	jmessage["msgtype"] = "getlist";
	jmessage["sequence"] = Sequence();
	SendMessage(jmessage);
}
...
bool PeerConnectionClient::HandleConnect(const Json::Value &jmessage)
{
	...
	else if (type == "getlist")
	{
		Json::Value user;
		bool ret;
		ret = rtc::GetValueFromJsonObject(jmessage, "terminal", &user);
		callback_->OnGetUserInfo(user);                      
	}
	else if (type == "getgroup")
	{
		Json::Value array1, array2, array3;
		bool ret;
		ret = rtc::GetValueFromJsonObject(jmessage, "group1", &array1);
		ret = rtc::GetValueFromJsonObject(jmessage, "group2", &array2);
		ret = rtc::GetValueFromJsonObject(jmessage, "group3", &array3);
		callback_->OnGetGroupInfo(array1, array2, array3);                    
	}
	else if (type == "state")
	{
