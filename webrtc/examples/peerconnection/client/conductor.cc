...
void Conductor::OnGetGroupInfo(Json::Value& group1, Json::Value& group2, Json::Value& group3)
{
	std::string table1 = "group1", table2 = "group2", table3 = "group3";
	main_proc_->DbDelGroup();
	if (group1.size() > 0)
		main_proc_->DbInsertGroup(group1, table1);
	if (group2.size() > 0)
		main_proc_->DbInsertGroup(group2, table2);
	if (group3.size() > 0)
		main_proc_->DbInsertGroup(group3, table3);
}

void Conductor::OnGetUserInfo(Json::Value& terminal)
{
	main_proc_->DbDelUserList();
	if (terminal.size() > 0)
		main_proc_->DbInsUserList(terminal);
}
