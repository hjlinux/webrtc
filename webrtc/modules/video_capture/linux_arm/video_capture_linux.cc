...
#if 1
    //_deviceChn = GetCaptureChn();
    _deviceChn = devChn;//GetCaptureChn();
    devChn ++;
    if(_deviceChn < 0)
    {
        WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, _id,
                     "Failed to GetCaptureChn");
        return -1;
            
    }
#if 0
    int VencFd = ZK_VIO_Init(_deviceChn);
    if(VencFd < 0)
    {
        exit(-1);
    }
...
bool VideoCaptureModuleV4L2::CaptureProcess()
{
...
#if 1
    videofd = getCaptureFd(_deviceChn);
    //printf("videofd:%d,chn:%d\n",videofd,_deviceChn);
    if(videofd <= 0) {
        usleep(25000);
    ...
    retVal = select(videofd + 1, &rSet, NULL, NULL, &timeout);
    if (retVal < 0 && errno != EINTR) // continue if interrupted
    {
        // select failed
        printf("video:%d select failed\n",_deviceChn);
        _captureCritSect->Leave();
//        StopInteraction2();
//        exit(0);
        return true;
    ...
    {
        // select timed out
        printf("video:%d timed out:%d\n",_deviceChn,videofd);
        _captureCritSect->Leave();
        return true;
    }
    else if (!FD_ISSET(videofd, &rSet))
    {
        // not event on camera handle
        printf("not event on camera handle\n");
        _captureCritSect->Leave();
        return true;
    }
    int ret;
    ...
    int num = 0;
    int len = buff.length;
    unsigned char *data = (unsigned char *)buff.start;
    if(NULL == data) {
        return true;    
    }
    offset += 5;
    while(1) {
        ret = readN(videofd,&frameLen,4);
        if(ret < 0) {
            _captureCritSect->Leave();
            return true;        
        }
        memcpy(data+offset,&frameLen,4);
        offset += 4;
        ret = readN(videofd,data+offset,frameLen);
        if(ret < 0) {
            _captureCritSect->Leave();
            return true;        
        }
//        printf("capture chn:%d,socket:%d,ntype:%d\n",_deviceChn,videofd,data[offset]);
#if 0
    if(!IsTeacher()) {
    if(stu_fd == 0) {
        stu_fd = open("/hejl/webrtc/student_r.h264",O_CREAT | O_TRUNC | O_WRONLY,0666);    
    }
    if(stu_fd > 0) {
        write(stu_fd,data+offset+1,frameLen-1);    
    }
    }
#endif
        num ++;
        if((data[offset] == 5) || (data[offset] == 1)) {
            offset += frameLen;
            break;     
        } else if((data[offset] >= 6) || (data[offset] <= 8)) {
            offset += frameLen;
        }
    }
    offset -= 4;
  //  printf("offset:%d\n",offset);
    memcpy(data,&offset,4);
    data[4] = num;
    //printf("capture,fd:%d,chn:%d,framelen:%d\n",videofd,_deviceChn,frameLen);
#endif
#if 1
    if(_captureStarted)
    {
        ...
        IncomingFrame((unsigned char*)data,len, frameInfo);
    }
#endif
//	free(data);
    _captureCritSect->Leave();
