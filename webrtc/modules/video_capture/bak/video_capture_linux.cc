    ...
    const VideoCaptureCapability& capability)
{
    ...
    _captureVideoType = (RawVideoType)0;
    _currentWidth = 1280;
    _currentHeight = 720;
    _captureDelay = 120;
...
if(!flag)
{
    fd = open("/p.yuv",O_CREAT | O_RDWR | O_TRUNC,0666);
if(fd < 0)
{
perror("open");
exit(0);
	...
	int len = 1382400, readBytes = 0;
	if(!image)
	{
	if(fd == 0)
	{
		fd = open("/video0.mjpg",O_RDONLY);
		if(fd < 0)
		{perror("open");exit(-1);}
	}
		lseek(fd,0,SEEK_SET);
...
#endif
	printf("\n%d %d %d\n",frameInfo.width,frameInfo.height,frameInfo.rawType);
	usleep(24000);
        IncomingFrame((unsigned char*)image,len, frameInfo);
    }
    //free(image);
    _captureCritSect->Leave();
    usleep(0);
