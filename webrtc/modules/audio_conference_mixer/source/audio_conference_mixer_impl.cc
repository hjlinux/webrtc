...
#if 0
void SaveFramesToBuff(AudioFrame* frame, voe::Channel *channel)
{
	unsigned int local_ssrc, remote_ssrc;
	FrameInfo frame_info;
	
	channel->GetLocalSSRC(local_ssrc);
    channel->GetRemoteSSRC(remote_ssrc);

    //printf("==-channel %p\n",channel);

    frame_info.local_ssrc = local_ssrc;
    frame_info.remote_ssrc = remote_ssrc;
    frame_info.num_channels = frame->num_channels_;
    frame_info.samples_per_channel = frame->samples_per_channel_;
    frame_info.sample_rate_hz = frame->sample_rate_hz_;

    SaveRemoteFrame(&frame_info, frame->data_);
	...
	//printf("--localssrc %u, remote ssrc %u, samples_per_channel_ %d,  sample_rate_hz_ %d, num_channels_ %d\n",
	       //local_ssrc, remote_ssrc, frame->samples_per_channel_, 
...
void AudioConferenceMixerImpl::CacheMixFrame(AudioFrame* frame, 
											MixerParticipant* mix_partic) const
{
	voe::Channel* channel = static_cast<webrtc::voe::Channel*>(mix_partic);
	voe::Channel* channel_iter;
	uint32_t ssrc;

	channel->GetLocalSSRC(ssrc);
	for (MixerParticipantList::const_iterator participant =
        _participantList.begin(); participant != _participantList.end();
         ++participant) {		
		if (mix_partic == *participant)
			continue;
		channel_iter = static_cast<webrtc::voe::Channel*>(*participant);
		channel_iter->SaveParticipantFrame(frame, ssrc);
    }
}
    ...
    for (MixerParticipantList::const_iterator participant =
        ...
        if(audioFrame->vad_activity_ == AudioFrame::kVadActive) {
            ...
            if(activeList.size() >= *maxAudioFrameCounter) {
                ...
                CacheMixFrame(audioFrame, *participant);
                //SaveFramesToBuff(audioFrame, 
                //                 static_cast<webrtc::voe::Channel*>(*participant));
            }
        } else {
            if(wasMixed) {
