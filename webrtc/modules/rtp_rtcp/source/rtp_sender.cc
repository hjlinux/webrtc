...
bool RTPSender::SendPacketToNetwork(const uint8_t* packet,
  ...
  if (transport_) {
#if 0
      unsigned char code[4] = {0,0,0,1};
    if((fd == 0) && IsTeacher() && getRecord()) {
        fd = open("/mnt/webrtc/rtp.h264",O_CREAT|O_WRONLY|O_TRUNC,0666);
    }
    if(fd && IsTeacher() && getRecord()) {
        write(fd,code,4);    
...
int32_t RTPSender::SendToNetwork(uint8_t* buffer,
  ...
  if (packet_history_.PutRTPPacket(buffer, length, capture_time_ms, storage) !=
      0) {
      //printf("packet_history_.PutRTPPacket\n");
    return -1;
  }
#endif
#if 0  //hejl
