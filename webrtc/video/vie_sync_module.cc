...
int64_t ViESyncModule::TimeUntilNextProcess() {
  const int64_t kSyncIntervalMs = 1000;//1000;  //hejl
...
void ViESyncModule::Process() {
  ...
  const int current_video_delay_ms = video_receiver_->Delay(); //hejl
  ...
  printf("target_video_delay_ms:%d,target_audio_delay_ms:%d\n",target_video_delay_ms,target_audio_delay_ms);
#if 0  //hejl
  target_audio_delay_ms = 0;
  target_video_delay_ms = 0;
#endif
...
bool ViESyncModule::GetStreamSyncOffsetInMs(const VideoFrame& frame,
  ...
  *stream_offset_ms = latest_audio_ntp - latest_video_ntp;
  printf("stream_offset_ms:%d\n",*stream_offset_ms);
  //*stream_offset_ms = 0;//latest_audio_ntp - latest_video_ntp; //hejl
  return true;
}
