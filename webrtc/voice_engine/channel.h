      ...
      public MixerParticipant  // supplies output mixer with audio frames
{
 public:
  ...
  int8_t MixAudioWithBuffer(int16_t target[], size_t target_channel);
  int8_t SaveParticipantFrame(AudioFrame* frame, uint32_t ssrc);
 ...
 private:
  ...
  typedef struct Audiocache_
  {
	#define AUDIO_CACHE_TIMEOUT 300
  	#define AUDIO_CACHE_SIZE    3
	uint32_t ssrc;
    uint32_t cnt;
    uint8_t index_r;
    uint8_t index_w;
    int16_t data[AUDIO_CACHE_SIZE][960];
  	uint16_t len;
  }AudioCache;
  AudioCache audio_cache_[3];
};
