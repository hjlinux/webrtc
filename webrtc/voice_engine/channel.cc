...
#if 0
int32_t MixAudioWithBuffer(int16_t target[],
                			size_t target_channel,
                			unsigned int local_ssrc) {
  FrameInfo frame_info;
  //int16_t frame_data[3840];
  std::unique_ptr<int16_t[]> frame_data(new int16_t[3840]);
  int32_t len;
  
  frame_info.local_ssrc = local_ssrc;
  if (GetRemoteFrame(&frame_info, frame_data.get()) >= 0)
  {
    len = frame_info.num_channels * frame_info.samples_per_channel;
    MixWithSat(target, target_channel, frame_data.get(),
               frame_info.num_channels, len);
  }
  return 0;
}
#endif
...
Channel::Channel(int32_t channelId,
  ...
  if (config.Get<NetEqCapacityConfig>().enabled) {
    ...
    acm_config.neteq_config.max_packets_in_buffer =
        std::max(10, config.Get<NetEqCapacityConfig>().capacity);//20
  }
  ...
  memset(audio_cache_, 0, sizeof(AudioCache) * 3);
}
...
uint32_t Channel::PrepareEncodeAndSend(int mixingFrequency) {
  ...
  //unsigned int remote_ssrc,local_ssrc;
  //GetRemoteSSRC(remote_ssrc);
  //GetLocalSSRC(local_ssrc);
  //printf("==remote ssrc %u,local ssrc %u, %p\n",remote_ssrc,local_ssrc,this);
  //MixAudioWithBuffer(_audioFrame.data_, _audioFrame.num_channels_,
  //              	 local_ssrc);
  MixAudioWithBuffer(_audioFrame.data_, _audioFrame.num_channels_);
...
int8_t Channel::MixAudioWithBuffer(int16_t target[], size_t target_channel)
{
	uint8_t i = 0;
	
	for (i = 0; i < 3; i++)
	{
		if (audio_cache_[i].index_r == audio_cache_[i].index_w)
		{
			audio_cache_[i].cnt++;
			if (audio_cache_[i].cnt > AUDIO_CACHE_TIMEOUT)
				audio_cache_[i].cnt = AUDIO_CACHE_TIMEOUT;
			continue;
		}
		audio_cache_[i].cnt = 0;
		MixWithSat(target, target_channel, audio_cache_[i].data[audio_cache_[i].index_r],
		           target_channel, audio_cache_[i].len);
		audio_cache_[i].index_r = (audio_cache_[i].index_r + 1) % AUDIO_CACHE_SIZE;
	}
	return 0;
}

int8_t Channel::SaveParticipantFrame(AudioFrame* frame, uint32_t ssrc) 
{
	uint8_t i = 0, index1 = -1, index2 = -1;
	uint16_t len = 0;
	
	for (i = 0; i < 3; i++)
	{
		if (audio_cache_[i].ssrc == ssrc)
		{
 			index1 = i;
 			break;
		}
		if (index2 < 0 && audio_cache_[i].cnt == AUDIO_CACHE_TIMEOUT
			&& audio_cache_[i].index_r == audio_cache_[i].index_w)
		{
			index2 = i;
		}
	}
	...
	len = frame->num_channels_ * frame->samples_per_channel_;
	memcpy(audio_cache_[i].data[audio_cache_[i].index_w], frame->data_, len * 2);
	audio_cache_[i].len = len;
	
	if (((audio_cache_[i].index_w + 1) % AUDIO_CACHE_SIZE) != audio_cache_[i].index_r)
	{
		audio_cache_[i].index_w = (audio_cache_[i].index_w + 1) % AUDIO_CACHE_SIZE;
	}
	return 0;
}
