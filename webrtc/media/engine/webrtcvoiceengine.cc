...
class WebRtcVoiceCodecs final {
 ...
 private:
  ...
  static const CodecPref kCodecPrefs[3];//12
...
const WebRtcVoiceCodecs::CodecPref WebRtcVoiceCodecs::kCodecPrefs[3] = {//12
    //{kOpusCodecName, 48000, 2, 111, true, {10, 20, 40, 60}, kOpusMaxBitrate},
...
WebRtcVoiceEngine::WebRtcVoiceEngine(webrtc::AudioDeviceModule* adm,
  ...
  // Set default engine options.
  {
    ...
    options.audio_jitter_buffer_max_packets = rtc::Optional<int>(20);//50
    options.audio_jitter_buffer_fast_accelerate = rtc::Optional<bool>(true);//false
    ...
    options.delay_agnostic_aec = rtc::Optional<bool>(true);//false
...
bool WebRtcVoiceEngine::ApplyOptions(const AudioOptions& options_in) {
  ...
  webrtc::EcModes ec_mode = webrtc::kEcAec;//kEcAecm
  webrtc::AecmModes aecm_mode = webrtc::kAecmLoudSpeakerphone;//kAecmSpeakerphone;
  webrtc::AgcModes agc_mode = webrtc::kAgcFixedDigital;//kAgcAdaptiveAnalog;
  ...
  if (options.echo_cancellation) {
    ...
    if (ec_mode == webrtc::kEcAecm)
    	printf("##use ec_mode == webrtc::kEcAecm.\n");
    else if (voep->SetEcMetricsStatus(*options.echo_cancellation) == -1) {
